...
<%
	...
	if (ClientHostType.ServerLocal.equals(clientHostType)){
		isLoginByLocal = true;
		request.setAttribute("isLoginByLocal",true);
	}else if (ClientHostType.OpenIdRelyParty.equals(clientHostType)){
...
$(document).ready(
	function(){
		...
		//验证码输入框的效果
		$("input#randomCode").addClass("colorGray").focus(function(){
			if($(this).val() == this.defaultValue){
				$(this).val("").removeClass("colorGray");
			}
		}).blur(function(){
	    		...
	    		<c:when test="${empty user}">
		    		<form id="loginForm" class="well" style="margin-top:35px;" data-widget="validator" action="${loginAction}" method="post">    			
				        <fieldset>
				          ...
				          <div class="control-group">
					          <input id="randomCode" name="randomCode" type="text" value="请输入验证码">
